...
public interface StudentTaskRepo extends JpaRepository<StudentTask, StudentTaskCompositeKey> {
  @Query("""
    ...
    where t.status = "Finished" and st.student.student_id = :student_id and s.sprint_id = :sprint_id and s.status = true
    group by t.task_id
""")
  ...
  @Query("""
      ...
      where t.status = :status and st.student.student_id = :student_id and pro.project_id = :project_id and pro.status = true
      group by t.task_id
  """)
